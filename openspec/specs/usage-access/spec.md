...
- **WHEN** `headroom usage` is called within the proxy process
...
- **WHEN** `headroom usage` is called directly (not within proxy) with `HEADROOM_API_KEY=hr_...`
...
- **WHEN** developer runs `headroom usage search "authentication" --last 30d`
